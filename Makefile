...
remove:
	# remove alias files
	# for file in $(shell find $(CURDIR) -type f -name ".*" -not -name ".gitignore" -not -name ".git" -not -name ".config" -not -name ".github" -not -name ".*.swp" -not -name ".gnupg" -not -name ".pytest_cache" -not -name ".DS_Store"); do \
	# 	f=$$(basename $$file); \
	# 	rm $(HOME)/$$f; \
	# done
...
dotfiles: ## Installs the dotfiles.
	...
	for file in $(shell find $(CURDIR) -type f -name ".*" -not -name "bash" -not -name ".gitignore" -not -name ".git" -not -name ".config" -not -name ".github" -not -name ".*.swp" -not -name ".gnupg" -not -name ".pytest_cache" -not -name ".DS_Store"); do \
		f=$$(basename $$file); \
		ln -sfn $$file $(HOME)/$$f; \
	done; \
	...
	for file in $(shell find $(CURDIR)/git/config -type f -not -name "*-backlight" -not -name ".*.swp"); do \
		f=$$(basename $$file); \
		ln -sfn $$file $(HOME)/.$$f; \
	done; \
	...
	## git update-index --skip-worktree $(CURDIR)/git/.gitconfig;
